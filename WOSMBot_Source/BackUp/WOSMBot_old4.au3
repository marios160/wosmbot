...
$WOSMBot = GUICreate("Bot do wywoływania/odkładania skutków magazynowych", 662, 530, -1036, 116)
...
Global $_ListaMagazynow[0]					;Zawiera liste dostepnych magazynow
...
Func btnStartClick()
	...
	For $i = 0 To UBound($_WybraneListyDoWykonania) - 1
		$el = StringSplit($_WybraneListyDoWykonania[$i], "[", 2)						;wyciaganie nazw list (sprzedaz detaliczna [MAG])
		Switch (StringStripWS($el[0], BitOR($STR_STRIPLEADING, $STR_STRIPTRAILING)))	;tworzenie listy skroconych nazw dokumentow dla bota
			Case "Sprzedaż detaliczna"
				$_WybraneListyDoWykonaniaBot[$i] = "PA"
			Case "Przchód wewnętrzny (Przyjęcia magazynowe)"
				$_WybraneListyDoWykonaniaBot[$i] = "PW"
			Case "Przesunięcie międzymagazynowe (Wydania magazynowe)"
				$_WybraneListyDoWykonaniaBot[$i] = "MM"
			Case "Rozchód wewnętrzny (Wydania magazynowe)"
				$_WybraneListyDoWykonaniaBot[$i] = "RW"
			Case "Zwroty detaliczne"
				$_WybraneListyDoWykonaniaBot[$i] = "ZW"
			Case Else
				$_WybraneListyDoWykonaniaBot[$i] = $el[0]
		EndSwitch
	Next
	Run($_SciezkaUruchomieniaSubiekta, "")												;uruchomienie subiekta

	While czySubiektUruchomiony() <> 0													;Czekamy az subiekt sie zaladuje
		Sleep(1000)
	WEnd
	Sleep(2000)
...
Func przygotowanieGUI()
	...
	ReDim $_ListaMagazynow[UBound($magTmp)]								;definiujemy tablice
	For $i = 0 To UBound($magTmp) - 1
		For $j = 0 To 7
			$_Magazyny[$i][$j] = 4
		Next
		If $magTmp[$i] <> "" Then $_ListaMagazynow[$i] = $magTmp[$i]
	Next

	GUICtrlSetData($cmbMagazyn, $_ListaMagazynow)						;ustawiamy liste dla wyboru magazynow
	$aktMagTxt = StringLeft($_ListaMagazynow[0], 3)						;ustawiamy aktualnie wybrany magazyn na pierwszy z listy
