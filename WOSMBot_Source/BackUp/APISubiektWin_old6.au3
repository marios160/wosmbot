...
Func odlozSkutekMagazynowy($lista)
	...
	While WinGetHandle("[TITLE:" & $nazwaOkna & "]") = 0 And WinGetHandle("[TITLE:Subiekt GT;CLASS:#32770]") = 0
		...
		$i = 0
		While (WinGetHandle("[TITLE:" & $nazwaOkna & "]") = 0 And WinGetHandle("[TITLE:Subiekt GT;CLASS:#32770]") = 0) And $i < 10
			ConsoleWrite("1 " & $nazwaOkna & @LF)
			If WinGetHandle("[TITLE:Subiekt GT;CLASS:#32770]") = 0 Then
				ControlSend(HWnd($_UchwytyKontrolek.Item($listaKopiuj).Item($CLS_GXWND)), "", "", "^+{Y}")
				If StringInStr(ClipGet(),$STR_ODLOZONY_SKUTEK) > 0 Then
					Return True
				EndIf
			EndIf
			$i = $i + 1
