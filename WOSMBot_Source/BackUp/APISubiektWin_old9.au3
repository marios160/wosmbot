...
Func ustawOkres($data, $lista)
	...
	;logs(@AutoItExe & ' /AutoIt3ExecuteLine "ControlCommand ( '''','''', hwnd(' & $_UchwytyKontrolek.Item($lista).Item($HWND_OKRES) & '), ''SelectString'', ''' & $POZ_DOWOLNY_OKRES & ''' )"')
	Do
...
Func pobierzUchwytKontrolkiListyPetla($hWnd, $broNM, $broCL, $childNM, $childCL)
	;logs('@@ (453) :(' & @MIN & ':' & @SEC & ') pobierzUchwytKontrolkiListyPetla()' & @CR) ;### Function Trace
	$wynik = 0
	While $wynik = 0
		$wynik = pobierzUchwytKontrolkiListy($hWnd, $broNM, $broCL, $childNM, $childCL)
		;logs($wynik & " " & $hWnd & " " & $broNM & " " & $broCL & " " & $childNM & " " & $childCL & @LF)
	WEnd
	Return $wynik
...
Func pobierzUchwytKontrolkiListy($hWnd, $broNM, $broCL, $childNM, $childCL)
	;logs('@@ (468) :(' & @MIN & ':' & @SEC & ') pobierzUchwytKontrolkiListy()' & @CR) ;### Function Trace
	Local $uchwyt = 0
	Local $hChild = _WinAPI_GetWindow($hWnd, $GW_CHILD)
	While $hChild
...
Func pobierzListeMagazynow()
	;logs('@@ (594) :(' & @MIN & ':' & @SEC & ') pobierzListeMagazynow()' & @CR) ;### Function Trace
...
Func pobierzUchwytMagazynow()
	;logs('@@ (608) :(' & @MIN & ':' & @SEC & ') pobierzUchwytMagazynow()' & @CR) ;### Function Trace
...
Func usunSeparatory($uchwyt)
	;logs('@@ (624) :(' & @MIN & ':' & @SEC & ') usunSeparatory()' & @CR) ;### Function Trace
...
Func zapiszUchwytFiltry($lista, $filtr)
	;logs('@@ (637) :(' & @MIN & ':' & @SEC & ') zapiszUchwytFiltry()' & @CR) ;### Function Trace
...
Func zapiszUchwytGXWND($lista)
	;logs('@@ (653) :(' & @MIN & ':' & @SEC & ') zapiszUchwytGXWND()' & @CR) ;### Function Trace
...
Func otworzFiltr($lista, $filtr)
	;logs('@@ (670) :(' & @MIN & ':' & @SEC & ') otworzFiltr()' & @CR) ;### Function Trace
	$hWnd = uchwytSubiekta()
...
Func ustawKolumnyListy($lista)
	;logs('@@ (819) :(' & @MIN & ':' & @SEC & ') ustawKolumnyListy()' & @CR) ;### Function Trace
...
Func restartujSubiekta()
	...
	MsgBox(0,"RESTART","RESTART")
	;ProcessClose("Subiekt.exe")
	ControlSend(uchwytSubiekta(),"","","!{F4}")
	Sleep(3000)
	$archiwizacja = 1
	Do
		$archiwizacja = oknoArchiwizacja()
...
Func MyErrFunc()
	logs('@@ (926) :(' & @MIN & ':' & @SEC & ') MyErrFunc()' & @CR) ;### Function Trace
