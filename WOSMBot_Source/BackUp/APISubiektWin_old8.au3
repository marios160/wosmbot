...
Func znajdzKomunikat()
	ConsoleWrite('@@ (125) :(' & @MIN & ':' & @SEC & ') znajdzKomunikat()' & @CR) ;### Function Trace
...
Func wywolajSkutekMagazynowy($lista)
	ConsoleWrite('@@ (143) :(' & @MIN & ':' & @SEC & ') wywolajSkutekMagazynowy()' & @CR) ;### Function Trace
	...
	EndSwitch
	$wiersz = False
	kopiujWierszSzybko($listaKopiuj)
	Do
		$i = 0
		$var = False
		...
		Do
			...
			EndIf
			znajdzKomunikat()
			$brakTowaru = WinGetHandle("[REGEXPTITLE:(?i)(.* - brak towaru w magazynie)]")
			If $brakTowaru <> 0 Then
				zapiszBrakTowaru($lista)
			EndIf
			$i = $i + 1
		Until  $i >= 1000 Or StringInStr(kopiujWierszSzybko($listaKopiuj),$STR_WYWOLUJE_SKUTEK) > 0
...
Func odlozSkutekMagazynowy($lista)
	ConsoleWrite('@@ (205) :(' & @MIN & ':' & @SEC & ') odlozSkutekMagazynowy()' & @CR) ;### Function Trace
	$nazwaOkna = ""
	$listaKopiuj = ""
	Switch ($lista)
		Case $LST_SPRZEDAZ_DETALICZNA
	...
	Do
		...
		Do
			...
			znajdzKomunikat()
			$i = $i + 1
		Until  $i >= 1000 Or StringInStr(kopiujWierszSzybko($listaKopiuj),$STR_ODLOZONY_SKUTEK) > 0
...
Func wyczyscFiltry($lista)
	ConsoleWrite('@@ (257) :(' & @MIN & ':' & @SEC & ') wyczyscFiltry()' & @CR) ;### Function Trace
...
Func getNumer($rekord)
	ConsoleWrite('@@ (267) :(' & @MIN & ':' & @SEC & ') getNumer()' & @CR) ;### Function Trace
...
Func wybierzListeNN($lista)
	ConsoleWrite('@@ (282) :(' & @MIN & ':' & @SEC & ') wybierzListeNN()' & @CR) ;### Function Trace
	Switch ($lista)
		Case $PA
...
Func wybierzListe($lista, $filtry = ObjCreate("Scripting.Dictionary"))
	ConsoleWrite('@@ (313) :(' & @MIN & ':' & @SEC & ') wybierzListe()' & @CR) ;### Function Trace
...
Func uchwytSubiekta()
	ConsoleWrite('@@ (382) :(' & @MIN & ':' & @SEC & ') uchwytSubiekta()' & @CR) ;### Function Trace
...
Func ustawOkres($data, $lista)
	ConsoleWrite('@@ (391) :(' & @MIN & ':' & @SEC & ') ustawOkres()' & @CR) ;### Function Trace
...
Func pobierzUchwytKontrolkiListyPetla($hWnd, $broNM, $broCL, $childNM, $childCL)
	ConsoleWrite('@@ (419) :(' & @MIN & ':' & @SEC & ') pobierzUchwytKontrolkiListyPetla()' & @CR) ;### Function Trace
	$wynik = 0
	While $wynik = 0
...
Func pobierzUchwytKontrolkiListy($hWnd, $broNM, $broCL, $childNM, $childCL)
	ConsoleWrite('@@ (434) :(' & @MIN & ':' & @SEC & ') pobierzUchwytKontrolkiListy()' & @CR) ;### Function Trace
	Local $uchwyt = 0
	Local $hChild = _WinAPI_GetWindow($hWnd, $GW_CHILD)
	While $hChild
...
Func czyPusta($lista)
	ConsoleWrite('@@ (463) :(' & @MIN & ':' & @SEC & ') czyPusta()' & @CR) ;### Function Trace
	Do
		$x = kopiujListe($lista)
	Until StringLen($x) > 0
	If StringInStr($x, $STR_ODLOZONY_SKUTEK) = 0 Then
...
Func czySubiektUruchomiony($_nazwaOknaSubiekta)
	ConsoleWrite('@@ (486) :(' & @MIN & ':' & @SEC & ') czySubiektUruchomiony()' & @CR) ;### Function Trace
...
Func zlapSubiekta($_nazwaOknaSubiekta)
	ConsoleWrite('@@ (525) :(' & @MIN & ':' & @SEC & ') zlapSubiekta()' & @CR) ;### Function Trace
...
Func wybierzFiltr($lista, $filtr, $pozycja = "")
	ConsoleWrite('@@ (542) :(' & @MIN & ':' & @SEC & ') wybierzFiltr()' & @CR) ;### Function Trace
	If $pozycja = "" Then
...
Func wybierzMagazyn($magazyn)
	ConsoleWrite('@@ (553) :(' & @MIN & ':' & @SEC & ') wybierzMagazyn()' & @CR) ;### Function Trace
...
Func pobierzListeMagazynow()
	ConsoleWrite('@@ (560) :(' & @MIN & ':' & @SEC & ') pobierzListeMagazynow()' & @CR) ;### Function Trace
...
Func pobierzUchwytMagazynow()
	ConsoleWrite('@@ (574) :(' & @MIN & ':' & @SEC & ') pobierzUchwytMagazynow()' & @CR) ;### Function Trace
...
Func usunSeparatory($uchwyt)
	ConsoleWrite('@@ (590) :(' & @MIN & ':' & @SEC & ') usunSeparatory()' & @CR) ;### Function Trace
...
Func zapiszUchwytFiltry($lista, $filtr)
	ConsoleWrite('@@ (603) :(' & @MIN & ':' & @SEC & ') zapiszUchwytFiltry()' & @CR) ;### Function Trace
...
Func zapiszUchwytGXWND($lista)
	ConsoleWrite('@@ (619) :(' & @MIN & ':' & @SEC & ') zapiszUchwytGXWND()' & @CR) ;### Function Trace
...
Func otworzFiltr($lista, $filtr)
	ConsoleWrite('@@ (636) :(' & @MIN & ':' & @SEC & ') otworzFiltr()' & @CR) ;### Function Trace
	$hWnd = uchwytSubiekta()
...
Func kopiujWiersz($lista)
	ConsoleWrite('@@ (686) :(' & @MIN & ':' & @SEC & ') kopiujWiersz()' & @CR) ;### Function Trace
	$j = 0
...
;~ 		ControlSend(HWnd($_UchwytyKontrolek.Item($lista).Item($CLS_GXWND)), "", "", "{CTRLDOWN}{SHIFTDOWN}Y{SHIFTUP}{CTRLUP}")
		...
		$j = $j + 1
	WEnd
	While _IsPressed(10) Or _IsPressed(11)
		Sleep(1)
ConsoleWrite('>Error code: ' & @error & @CRLF & @CRLF & '@@ Trace(702) :    		Send("{SHIFTUP}")'  & @CRLF) ;### Trace Console
		Send("{SHIFTUP}")
		Send("{CTRLUP}")
	WEnd
	Return ClipGet()
...
Func kopiujWierszSzybko($lista)
	...
	While _IsPressed(10) Or _IsPressed(11)
		Sleep(1)
ConsoleWrite('>Error code: ' & @error & @CRLF & @CRLF & '@@ Trace(715) :    		Send("{SHIFTUP}")'  & @CRLF) ;### Trace Console
		Send("{SHIFTUP}")
		Send("{CTRLUP}")
	WEnd
	Return ClipGet()
...
Func kopiujListe($lista)
	ConsoleWrite('@@ (722) :(' & @MIN & ':' & @SEC & ') kopiujListe()' & @CR) ;### Function Trace
	ClipPut("XEDEX_L")
...
Func czekajNaZaladowanieListy($lista)
	ConsoleWrite('@@ (738) :(' & @MIN & ':' & @SEC & ') czekajNaZaladowanieListy()' & @CR) ;### Function Trace
...
Func fokusNaGXWND($lista)
	ConsoleWrite('@@ (752) :(' & @MIN & ':' & @SEC & ') fokusNaGXWND()' & @CR) ;### Function Trace
	$hWnd = 0
	While $hWnd = 0
...
Func komunikatAktualizacji()
	ConsoleWrite('@@ (768) :(' & @MIN & ':' & @SEC & ') komunikatAktualizacji()' & @CR) ;### Function Trace
	$hWnd = WinGetHandle('[REGEXPTITLE:(?i)(Informacja o nowej wersji: *)]')
	If $hWnd <> 0 Then
		$uchwyt = szukajUchwytu($hWnd, "&Zamknij", $CLS_BUTTON)
		ControlClick($uchwyt, "", "")
...
Func ustawKolumnyListy($lista)
	ConsoleWrite('@@ (780) :(' & @MIN & ':' & @SEC & ') ustawKolumnyListy()' & @CR) ;### Function Trace
	...
	While $tmp2[0] <> "S" Or $tmp2[1] <> "Data" Or StringReplace($tmp2[2],@CRLF,"") <> "Numer"
		ConsoleWrite($kopiaListy)
		$hWnd = 0
		While $hWnd = 0
			ControlSend(HWnd($_UchwytyKontrolek.Item($lista).Item($CLS_GXWND)), "", "", "{CTRLDOWN}{SHIFTDOWN}k{CTRLUP}{SHIFTUP}")
			$hWnd = WinGetHandle("Lista kolumn")
		WEnd
		MsgBox(0,"Wybór kolumn - " & $lista,"Proszę wybrać kolumny do wyświetlenia w kolejności:" & @LF & "- Status dokumentu" & @LF & "- Data wystawienia" & @LF & "- Numer")
		While $hWnd <> 0
			$hWnd = WinGetHandle("Lista kolumn")
		WEnd
		$kopiaListy = kopiujListe($lista)
...
Func pobierzUchwytFiltrow()
	ConsoleWrite('@@ (805) :(' & @MIN & ':' & @SEC & ') pobierzUchwytFiltrow()' & @CR) ;### Function Trace
	ControlFocus("", "", uchwytSubiekta())
