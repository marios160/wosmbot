	...
	For $i = 0 To UBound($_WybraneListyDoWykonania) - 1
		...
		Switch (StringStripWS($el[0], BitOR($STR_STRIPLEADING, $STR_STRIPTRAILING))) ;tworzenie listy skroconych nazw dokumentow dla bota
			Case "Sprzedaż detaliczna"
				$_WybraneListyDoWykonaniaBot[$i] = $LST_SPRZEDAZ_DETALICZNA
		...
		EndSwitch
	Next
	;Run($_SciezkaUruchomieniaSubiekta, "", @SW_HIDE) ;uruchomienie subiekta
...
Func przygotowanieGUI()
	...
	GUICtrlSetData($cmbMagazyn, _ArrayToString($magTmp),0) ;ustawiamy liste dla wyboru magazynow
