...
GUICtrlSetState($radioWywolywanie, $GUI_CHECKED)
GUICtrlSetOnEvent($radioWywolywanie, "radioWywolywanieClick")
GUICtrlCreateGroup("", -99, -99, 1, 1)
$Group2 = GUICtrlCreateGroup("Data", 8, 80, 225, 97)
$dataOd = GUICtrlCreateDate("2018/09/11 10:10:35", 48, 104, 162, 21, $DTS_SHORTDATEFORMAT) ;08-11
GUICtrlSetOnEvent($dataOd, "dataOdChange")
$dataDo = GUICtrlCreateDate("2018/09/20 10:10:39", 48, 136, 162, 21, $DTS_SHORTDATEFORMAT)
...
If FileExists("ListaDoWykonania.xml") Then
	$hF = FileOpen("ListaDoWykonania.xml")
	...
	For $i = 0 To UBound($tmp) - 1
		...
		If $tmp[$i] <> "" Then
			If StringLen($tmp[$i]) = 4 Then
				...
				$aktMag = $index - 1
				ConsoleWrite(@error & " " & StringLeft($tmp[$i], 3) & " " & $index & @LF)
				_GUICtrlComboBox_SetCurSel($cmbMagazyn, $index)
			Else
				ConsoleWrite(StringLeft($tmp[$i], 22) & " " & StringLen($tmp[$i]) & @LF)
...
Func btnStartClick()
	...
	;wklejanie tresci do pliku konfiguracyjnego
	$hFile = FileOpen("ListaDoWykonania.xml", 2 + 512)
