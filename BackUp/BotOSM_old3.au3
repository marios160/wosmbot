...
Func startOSMBot($dane, $dokumenty, $magazyny)
...
;~ 	Global Enum $MAG = 0, $ABR = 1, $KUN = 2, $TAR = 3
	...
	filtryDomyslne(_ArrayUnique($dokumenty))
			...
			While $endpos <> $curpos
				...
				If StringInStr($curpos, "wywołuje skutek magazynowy") <> 0 Then
					While Not $czyDaSieOdlozyc
						...
						If StringInStr($curpos2, "wywołuje skutek magazynowy") <> 0 And Not $czyDaSieOdlozyc Then
...
;~ 		 WEnd
			WEnd
			If StringInStr(kopiujListe(), "wywołuje") <> 0 Then
				StringReplace(kopiujListe(), "wywołuje skutek magazynowy", "xxx")
				logs("OMINIETO PRODUKT! " & "; " & " znaleziono: " & @extended)
			EndIf
		Until StringInStr(kopiujListe(), "wywołuje") = 0
	EndIf
	logs("------------------------------- KONIEC " & $lista & " ----------------------------------------")
