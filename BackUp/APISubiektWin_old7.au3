...
Func odlozSkutekMagazynowy($lista)
	...
	EndSwitch
	$wiersz = ""
	While StringInStr($wiersz, $STR_ODLOZONY_SKUTEK) = 0
		WinMenuSelectItem(uchwytSubiekta(), "", $MNU_OPERACJE, $SUB_ODLOZ_SKUTEK_MAGAZYNOWY)
		$i = 0
		While StringInStr($wiersz, $STR_ODLOZONY_SKUTEK) = 0 And $i < 1000
			znajdzKomunikat()
			$i = $i + 1
			$wiersz = kopiujWierszSzybko($listaKopiuj)
		WEnd
		ConsoleWrite(kopiujWierszSzybko($listaKopiuj))
		$wiersz = kopiujWierszSzybko($listaKopiuj)
	WEnd
	Return True
...
Func kopiujWiersz($lista)
	...
	$j = 0
	ClipPut("XEDEX_W")
	While ClipGet() = "XEDEX_W" And $j < 1000
		$i = 0
...
Func kopiujWierszSzybko($lista)
	ConsoleWrite('@@ (610) :(' & @MIN & ':' & @SEC & ') kopiujWierszSzybko()' & @CR) ;### Function Trace
	ControlFocus(HWnd($_UchwytyKontrolek.Item($lista).Item($CLS_GXWND)), "", "")
	Send("^+{Y}")
	Return ClipGet()
EndFunc   ;==>kopiujWiersz
